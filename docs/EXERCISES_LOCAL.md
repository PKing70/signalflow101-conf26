# SignalFlow 101 Exercises — Local Python Path
### Workshop Exercise Guide — .conf26 DEV1942
...
- A browser
- Git
- A Python environment that was already working before the workshop
- The workshop handout, including this GitHub repo link, the O11y workshop link,
  and your assigned `PARTICIPANT_ID`

Use this path only if your own Python environment was already working before the
workshop. We will not troubleshoot laptop-specific Python, firewall, package
manager, or IDE issues during the 60-minute session.
...
This is the existing local Python version of the workshop. Stay in this file for
setup, Exercise 1, Exercise 2, Exercise 3, and the take-home exercises. The guide
shows both Mac/Linux and Windows PowerShell commands.

To stop a running script, press **Ctrl+C** in its terminal.
...
5. Click the copy button for **Token Secret**. You will paste that clipboard
   value into `SPLUNK_INGEST_TOKEN` in your local `.env` file.
...
8. Click the copy button for **Token Secret**. You will paste that clipboard
   value into `SPLUNK_API_TOKEN` in your local `.env` file.
...
## Step 0A: Get The Workshop Files
...
## Step 0B: Configure Workshop Values
...
file, then press **Ctrl+X** to exit. If you use Notepad, edit the values, save,
and close the file.
...
Use token secrets, not token IDs. Paste the copied Token Secret values into
`.env`; do not paste them into Python files. Your participant ID is not copied
from O11y or your laptop; it is assigned by workshop staff.
...
.venv/bin/python workshop.py check
```
...
.\.venv\Scripts\python workshop.py check
```
...
```bash
cd ~/workshops/signalflow101-conf26
.venv/bin/python exercises-python/exercise1.py
```
...
```powershell
Set-Location "$HOME\workshops\signalflow101-conf26"
.\.venv\Scripts\python exercises-python\exercise1.py
```
...
the `.env` file you configured earlier.
...
```bash
cd ~/workshops/signalflow101-conf26
.venv/bin/python exercises-python/exercise2a.py
```
...
```powershell
Set-Location "$HOME\workshops\signalflow101-conf26"
.\.venv\Scripts\python exercises-python\exercise2a.py
```
...
```bash
cd ~/workshops/signalflow101-conf26
.venv/bin/python exercises-python/exercise2b.py
```
...
```powershell
Set-Location "$HOME\workshops\signalflow101-conf26"
.\.venv\Scripts\python exercises-python\exercise2b.py
```
...
```bash
cd ~/workshops/signalflow101-conf26
.venv/bin/python exercises-python/exercise3.py
```
...
```powershell
Set-Location "$HOME\workshops\signalflow101-conf26"
.\.venv\Scripts\python exercises-python\exercise3.py
```
