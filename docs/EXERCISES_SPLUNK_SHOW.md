# SignalFlow 101 Exercises — Splunk Show SSH Path
...
- A browser
- An SSH-capable terminal
- The workshop handout, including this GitHub repo link, the Splunk Show link,
  the O11y workshop link, and your assigned `PARTICIPANT_ID`

Splunk Show provides the Python environment. You do not need to install Python
...
This is the Splunk Show SSH/CLI version of the workshop. Stay in this file for
setup, Exercise 1, Exercise 2, Exercise 3, and the take-home exercises. All
commands run inside your Splunk Show SSH terminal unless this guide explicitly
says to use your browser.
...
## Step 0B: Get The Workshop Files
...
## Step 0C: Configure Workshop Values

From the repo directory in your Splunk Show SSH terminal, create your `.env`
file:
...
Open `.env` in a text editor. Use any editor you already know. If you are
working only in a terminal, `nano .env` opens it there.
...
from O11y or Splunk Show; it is assigned by workshop staff.

## Step 0D: Verify Setup
...
The files already exist in the repo. Run the `python ...` commands shown from the repo root in your Splunk Show SSH terminal, then use the code listings to understand what each script does.
...
If your Splunk Show instance is still available, SSH into it and go to the repo directory:
...
Replace the workshop values with values from your own Splunk Observability Cloud org:
